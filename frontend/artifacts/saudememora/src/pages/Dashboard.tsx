...
import {
  ArrowUpRight, ChevronRight, FileCheck2, FilePlus2,
    ...
    <div className="page-enter space-y-7">
        ...
        <div className="relative z-10 flex flex-col justify-between gap-6 md:flex-row md:items-center">
          <div className="max-w-2xl">
            ...
            <p className="mt-2 text-sm text-white/80 leading-relaxed max-w-xl">
              Todos os seus documentos de saúde em uma única tabela unificada. A IA lê e classifica cada documento automaticamente.
            </p>
          ...
          <div className="flex flex-wrap items-center gap-3 shrink-0">
            <button
              ...
              className="flex items-center gap-2 rounded-2xl bg-white px-5 py-3 text-xs font-extrabold text-primary shadow-md hover:bg-white/90 hover:-translate-y-0.5 transition-all cursor-pointer"
            >
      ...
      <section className="grid gap-4 sm:grid-cols-3">
        ...
        <div className="flex items-center gap-4 rounded-2xl border border-border bg-card p-5">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground">Histórico preenchido</p>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-border bg-card p-5 md:p-6 shadow-xs">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border/70 pb-4">
          ...
          <Link href="/documentos" className="flex items-center gap-1.5 text-xs font-bold text-primary hover:text-accent transition-colors">
            <span>Ver em Meus Documentos ({documents.length})</span>
        ...
        {documents.length === 0 ? (
          <div className="mt-6 flex flex-col items-center gap-3 rounded-2xl border border-dashed border-border bg-muted/20 p-8 text-center">
            ...
            <div>
              <p className="text-sm font-bold">Nenhum documento cadastrado ainda</p>
              <p className="mt-1 text-xs text-muted-foreground">Envie qualquer exame, receita, laudo ou comprovante. A IA detectará automaticamente o tipo do documento.</p>
            </div>
            <button
              onClick={() => triggerUploadModal()}
            ...
            >
              <Plus size={15} /> Adicionar Primeiro Documento
            </button>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-border text-[11px] font-bold text-muted-foreground uppercase tracking-wider">
                  ...
                  <th className="py-3 px-3">Resumo da IA</th>
                  ...
                  <tr key={doc.id} className="hover:bg-muted/40 transition-colors group">
                    <td className="py-3.5 px-3 font-bold text-foreground">
                      <Link href={`/documentos/${doc.id}`} className="hover:text-primary transition-colors flex items-center gap-2">
                        ...
                        <span className="truncate max-w-[180px]">{doc.title || 'Documento sem título'}</span>
                      </Link>
                    </td>
                    ...
                    <td className="py-3.5 px-3 text-muted-foreground max-w-[260px]">
      ...
      <section className="grid gap-5 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border border-border bg-card p-5 md:p-6 space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider block mb-2">Alergias</span>
              {allergies.length > 0 ? (
            ...
            <div>
              <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider block mb-2">Doenças / Condições</span>
        ...
        <div className="rounded-2xl border border-border bg-card p-5 md:p-6 space-y-4">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <div className="rounded-xl bg-muted/40 p-3 text-center">
              ...
              <span className="text-sm font-black text-red-600 dark:text-red-400 mt-1 block">{bloodType || '—'}</span>
            </div>
            <div className="rounded-xl bg-muted/40 p-3 text-center">
