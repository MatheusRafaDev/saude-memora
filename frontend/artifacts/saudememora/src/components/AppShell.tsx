...
const navItems = [
  ...
  { href: '/anamnese', label: 'Minha anamnese', icon: BookOpen },
    ...
    <div className="app-noise min-h-[100dvh] bg-background">
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-[260px] flex-col bg-[hsl(var(--sidebar))] px-4 py-5 text-[hsl(var(--sidebar-foreground))] transition-transform duration-300 md:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex items-center justify-between px-3">
          <Link href="/painel" className="flex items-center gap-3" data-testid="link-brand">
        ...
        <div className="mt-auto border-t border-white/10 pt-4">
          <div className="flex items-center gap-3 px-2">
            ...
            <button onClick={() => { signOut(); navigate('/entrar'); }} aria-label="Sair" data-testid="button-signout" className="rounded-lg p-2 text-white/40 hover:bg-white/10 hover:text-white"><LogOut size={15} /></button>
          </div>
        </div>
      </aside>
      ...
      <div className="min-h-[100dvh] md:pl-[260px]">
        <header className="sticky top-0 z-20 flex h-[68px] items-center justify-between border-b border-border/70 glass-header px-5 md:px-10">
          ...
          <div className="ml-auto flex items-center gap-3">
            <span className="hidden font-mono text-[10px] uppercase tracking-[.12em] text-muted-foreground sm:block">{currentDate}</span>
